...
## Preferred Execution: Driver Script

The recommended way to run superconductor is via the driver script, which controls the outer loop from bash and invokes the agent per-track via `codex exec`. This prevents the agent from declaring partial completion.

```bash
# Full run (setup + execution):
bash scripts/superconductor-driver.sh <repo-root> --setup <requirements-source>

# Resume existing run:
bash scripts/superconductor-driver.sh <repo-root>

# Dry run:
bash scripts/superconductor-driver.sh <repo-root> --dry-run
```

The driver handles: state detection, track batching, codex invocation, gate checking, and loop control. The agent only needs to launch it.
...
4. **The driver controls the loop.** The agent does not decide when to stop. The bash driver runs completion-gate.sh after every wave and only exits on gate pass.
