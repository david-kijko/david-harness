...
try:
    ...
    print(
        "exa_cli.py requires `exa-py`. Install it with `uv pip install exa-py` "
        "or run via `uv run --with exa-py python ~/.codex/skills/exa/scripts/exa_cli.py ...`.",
        file=sys.stderr,
    )
...
def load_api_key() -> str:
    ...
    if api_key:
        return api_key
    raise CliError(
        "Missing EXA_API_KEY. Export it in your shell before running the Exa skill.",
        exit_code=2,
    )
