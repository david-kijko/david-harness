...
- `scripts/uncommon_sense.py` also generates Claude agent team briefs from the intent contract and task ledger.
...
- selective parallel -> create a small Claude agent team
- full swarm -> create a larger Claude agent team with synthesis and verification roles
...
In Claude Code, prefer agent teams for fan-out because each teammate gets an independent context window and a shared task list. This reduces pressure on the lead context window, although the official docs note it increases total token usage.
...
- when using Claude agent teams, have the lead assign or seed the shared task list before teammates self-claim
...
```bash
python3 ~/.claude/skills/uncommon-sense/scripts/uncommon_sense.py build-artifact \
  --intent-contract /path/to/intent_contract.json \
...
```bash
python3 ~/.claude/skills/uncommon-sense/scripts/uncommon_sense.py telemetry-snapshot \
  --intent-contract /path/to/intent_contract.json \
  --task-ledger /path/to/task_ledger.json \
  --synthesis-state /path/to/synthesis_state.json \
