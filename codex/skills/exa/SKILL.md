...
```bash
python3 ~/.codex/skills/exa/scripts/exa_cli.py run --query "<rewritten query>" --format compact-markdown
```

If `exa-py` is not installed in the default interpreter, the CLI will automatically retry through `uv` when `uv` is available.

If you want an explicit no-install fallback, use:

```bash
uv run --with exa-py python3 ~/.codex/skills/exa/scripts/exa_cli.py run --query "<rewritten query>" --format compact-markdown
```

Persist the API key in:
