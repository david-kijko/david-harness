...
| Mode | Flag | Use When |
|------|------|----------|
| `dangerous` | (default) | All tasks. Uses `--dangerously-bypass-approvals-and-sandbox` so Hephaestus can read files and run commands without bwrap blocking. |
| `workspace-write` | `--full-auto` | Alias kept for compatibility, but dangerous is default now. |

**Default is dangerous (bypass sandbox).** The bwrap sandbox on this system blocks all file reads and bash commands, making read-only and workspace-write modes non-functional. The `--dangerously-bypass-approvals-and-sandbox` flag is required for Hephaestus to work at all.

## Heartbeat Monitoring

**CRITICAL: When invoking Hephaestus, Claude MUST maintain an active heartbeat.**

Do NOT use `run_in_background` and forget. Instead:
1. Run hephaestus in the background
2. Poll the output file every 30 seconds with `tail -5`
3. Report progress to the user at each poll
4. When the output file appears and the process exits, read and present the full result immediately

Example monitoring loop:
```bash
# After launching in background, poll:
while ! ls /home/david/.hephaestus/outputs/hephaestus-TIMESTAMP.md 2>/dev/null; do
  sleep 30
  tail -3 /tmp/claude-.../tasks/TASKID.output 2>/dev/null
done
# Output ready — read it
cat /home/david/.hephaestus/outputs/hephaestus-TIMESTAMP.md
```

Do NOT tell the user "I'll let you know when it's done" and then go silent. The user WILL have to ask. Poll proactively.
